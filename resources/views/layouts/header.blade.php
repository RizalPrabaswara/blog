            ...
            <x-dropdown>
                ...
                <x-slot name="trigger">
                    <button class="py-2 pl-3 pr-9 text-sm font-semibold w-full lg:w-32 text-left lg:inline-flex">
                        ...
                        <x-icon name="down-arrow" class="absolute pointer-events-none" style="right: 10px;" />

                    </button>
                </x-slot>

                <x-dropdown-item href="/posts" :active="request()->Routeis('home')"> All
                </x-dropdown-item>
                @foreach ($categories as $category)
                    <x-dropdown-item href="/categories/{{ $category->slug }}" :active="request()->is('categories/' . $category->slug)">
                        {{-- isset($currentCategory) && $currentCategory->is($category) Cara Lain untuk Isset --}}
